...
class Blizzard:
    ...
    def game_mythic_keystone_season_index(self):
        ...
        if self.current_season.status_code == 200: return json.loads(self.current_season.text)
        else: return None
    ...
    def game_profession_index(self):
        ...
        if self.p_name.status_code == 200: return json.loads(self.p_name.text)
        else: return None
    ...
    def game_profession(self, n):
        ...
        if self.p_tier.status_code == 200: return json.loads(self.p_tier.text)
        else: return None
    ...
    def game_mythic_keystone_dungeons_index(self):
        ...
        if self.dungeon.status_code == 200: return json.loads(self.dungeon.text)
        else: return None

    def game_mythic_keystone_dungeons(self, index):
        self.api_count += 1
        self.dungeon = requests.get("https://us.api.blizzard.com/data/wow/mythic-keystone/dungeon/" + str(index) + "?namespace=dynamic-us&locale=en_US&access_token=" + self.access_token)
        if self.dungeon.status_code == 200: return json.loads(self.dungeon.text)
        else: return None
    ...
    def game_playable_specialization(self, id):
        ...
        if self.spec.status_code == 200: return json.loads(self.spec.text)
        else: return None
    ...
    def game_mythic_keystone_seasons(self):
        self.api_count += 1
        self.key_season = requests.get("https://us.api.blizzard.com/data/wow/mythic-keystone/season/index?namespace=dynamic-us&locale=en_US&access_token=" + self.access_token)
        if self.spec.status_code == 200: return json.loads(self.key_season.text)
        else: return None
    ...
    def profile_character_equipment(self, player, server):
        ...
        if self.p_gear.status_code == 200: return json.loads(self.p_gear.text)
        else: return None
    ...
    def profile_character_profession(self, name, server):
        ...
        if self.p_player.status_code == 200: return json.loads(self.p_player.text)
        else: return None
    ...
    def profile_character_profile(self, name, server):
        ...
        if self.profile.status_code == 200: return json.loads(self.profile.text)
    ...
    def profile_character_pvp(self, name, server, bracket):
        ...
        if self.pvp.status_code == 200: return json.loads(self.pvp.text)["rating"]
        else: return 0
    ...
    def profile_character_media(self, name, server):
        ...
        if self.render.status_code == 200: return json.loads(self.render.text)
        else: return None

    def profile_specialization(self, url):
        self.api_count += 1
        self.spec = requests.get(url)
        if self.spec.status_code == 200: return json.loads(self.spec.text + "&access_token=" + self.access_token)
        else: return None
    ...
    def generic_call(self, url):
        ...
        if self.external.status_code == 200: return json.loads(self.external.text)
        else: return None
